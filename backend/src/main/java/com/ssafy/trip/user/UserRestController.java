...
public class UserRestController {
     ...
     * 
     * @param session HTTP 세션
     * @return 로그아웃 결과 및 상태 코드
     */
    @Operation(summary = "User logout", description = "Invalidates the user session")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> updateUser(@RequestBody User user, HttpSession session) {
        ...
        try {
            int result = userService.updateUser(user.getEmail(), user.getUsername(), user.getPassword());
            if (result > 0) {
                // 세션 정보 업데이트
    ...
    public ResponseEntity<?> changePassword(@RequestBody Map<String, String> passwordRequest, HttpSession session) {
        ...
        try {
            ...
            int result = userService.updateUser(email, user.getUsername(), newPassword);
            if (result > 0) {
                response.put("success", true);
