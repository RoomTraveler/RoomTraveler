<template>
  <div class="container mt-5 mb-5">
    ...
    <div v-if="loadingInitial" class="text-center py-5">
      <div class="spinner-border text-primary" role="status"><span class="visually-hidden">데이터 로딩 중...</span></div>
    </div>
    <div v-else-if="initialError" class="alert alert-danger">{{ initialError }}</div>
    <div v-else-if="!hostId || hostStatus !== 'APPROVED'" class="alert alert-warning text-center">
    ...
    <div v-else>
      ...
      <div class="card card-body mb-4 shadow-sm">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-3">
            ...
            <input type="date" id="filterCheckInDate" class="form-control form-control-sm" v-model="filters.checkInDate">
          </div>
           <div class="col-md-3">
        ...
        <div class="row g-3 mt-2">
            <div class="col-md-6">
                ...
                <input type="text" id="filterGuestName" class="form-control form-control-sm" v-model="filters.guestName" placeholder="게스트 이름으로 검색...">
            </div>
      ...
      <div v-if="loadingReservations" class="text-center py-4">
        <div class="spinner-border text-secondary" role="status"><span class="visually-hidden">예약 목록 로딩 중...</span></div>
      </div>
      ...
      <div v-else>
        ...
        <nav v-if="totalPages > 1" aria-label="Page navigation" class="d-flex justify-content-center mt-4">
          <ul class="pagination pagination-sm">
            ...
            <li v-for="pageNumber in pageRange" :key="pageNumber.toString()" class="page-item" :class="{ active: currentPage === pageNumber, disabled: pageNumber === '...' }">
              <a v-if="pageNumber !== '...'" class="page-link" href="#" @click.prevent="changePage(pageNumber)">{{ pageNumber }}</a>
    ...
    <!-- 예약 상태 변경 모달 -->
    <div v-if="selectedReservation" class="modal fade show d-block" tabindex="-1" style="background-color: rgba(0,0,0,0.5);" @click.self="closeUpdateModal">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>
              <strong>숙소:</strong> {{ selectedReservation.accommodationTitle }} ({{ selectedReservation.roomName }})<br>
            ...
            <div class="mb-3">
              ...
              <select id="newStatus" class="form-select" v-model="newStatus">
                ...
                <option v-if="selectedReservation.status === 'PENDING' || selectedReservation.status === 'CONFIRMED' || selectedReservation.status === 'CANCELLED_REQUEST'" value="CANCELLED">예약취소</option>
            ...
            <div v-if="newStatus === 'CANCELLED' && (selectedReservation.status === 'PENDING' || selectedReservation.status === 'CONFIRMED' || selectedReservation.status === 'CANCELLED_REQUEST')" class="mb-3">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" @click="submitUpdateStatus" :disabled="!newStatus || statusUpdateLoading">
              <span v-if="statusUpdateLoading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
              변경 저장
            </button>
          </div>
        </div>
...
async function fetchReservations() {
    if (!hostId.value || hostStatus.value !== 'APPROVED') {
    ...
    try {
        ...
        const response = await api.get(`/api/host/${hostId.value}/reservations?${params.toString()}`);
