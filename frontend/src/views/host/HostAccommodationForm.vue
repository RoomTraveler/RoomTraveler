    ...
    <el-form :model="form" :rules="rules" ref="accommodationFormRef" label-position="top" @submit.prevent="submitForm">
      ...
      <el-form-item label="편의시설" prop="amenities">
        <el-checkbox-group v-model="form.amenities">
          <el-checkbox v-for="item in form.availableAmenities" :key="item.key" :value="item.key">{{
            item.label
          }}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
onMounted(async () => {
  ...
  try {
    ...
    if (isEditMode.value && accommodationId.value) {
      ...
      if (response) {
        // response가 falsy가 아니면 성공으로 간주 (실제로는 response.success 등으로 확인 필요)
...
async function populateFormWithExistingData(apiData) {
  // 파라미터 이름을 apiData로 변경하여 명확화
  ...
  form.amenities = Array.isArray(data.amenities)
    ? data.amenities
    : typeof data.amenities === "string"
      ? JSON.parse(data.amenities)
      : []; // amenities가 JSON 문자열로 올 경우 대비
...
const submitForm = async () => {
  ...
  await accommodationFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const currentHostId = userStore.user?.hostId; // userStore에서 hostId 가져오기
        if (!currentHostId && !isEditMode.value) {
          // 새 숙소 등록 시 hostId가 없으면 에러 처리
          ElMessage.error("호스트 정보를 찾을 수 없습니다. 다시 로그인하거나 호스트 등록을 확인해주세요.");
          isSubmitting.value = false;
          return;
        }

        const fd = new FormData();
        // isEditMode가 아닐 때만 (새 숙소 등록 시) hostId를 추가합니다.
        // 수정 시에는 accommodationId로 숙소를 식별하므로 hostId를 폼 데이터에 넣을 필요는 없을 수 있습니다.
        // (백엔드 API 설계에 따라 다름. 필요하다면 수정 시에도 추가)
        if (!isEditMode.value && currentHostId) {
          fd.append("hostId", currentHostId.toString());
        }
        ...
        if (form.mainImage instanceof File) {
          ...
          if (isEditMode.value && existingMainImageId && existingMainImageId !== form.mainImage.name) {
            // 기존 대표 이미지가 있었고, 새 파일로 교체되면
        ...
        } else if (isEditMode.value && existingMainImageId) {
          // 새 대표 이미지 파일 없고, 기존 대표 이미지가 있었으면
        ...
        if (response && response.success) {
          ElNotification.success({
            ...
            message:
              response.message ||
              (isEditMode.value ? "숙소 정보가 수정되었습니다." : "숙소 등록 요청이 완료되었습니다."),
          });
