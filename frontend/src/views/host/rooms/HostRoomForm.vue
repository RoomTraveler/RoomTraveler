<template>
  <div class="room-form-container p-4 bg-white shadow-sm rounded">
    ...
    <el-form :model="form" :rules="rules" ref="roomFormRef" label-position="top" @submit.prevent="submitFormAction">
      <el-row :gutter="20">
        <el-col :span="12">
      ...
      <el-form-item label="객실 설명" prop="description">
        <el-input
          ...
          placeholder="객실의 특징, 침대 타입 등을 상세히 설명해주세요."
        />
      </el-form-item>
      ...
      <el-form-item label="편의시설 (객실 내)" prop="amenities">
        <el-checkbox-group v-model="form.amenities">
          <el-checkbox v-for="item in availableAmenities" :key="item.key" :value="item.key" :label="item.label">{{
            item.label
          }}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
const rules = {
  ...
  capacity: [
    { required: true, type: "integer", min: 1, message: "기준 인원은 1명 이상이어야 합니다.", trigger: "blur" },
  ],
  maxCapacity: [
    { required: true, type: "integer", message: "최대 인원을 입력해주세요.", trigger: "blur" },
    {
  ...
  roomCount: [
    { required: true, type: "integer", min: 1, message: "객실 수는 1개 이상이어야 합니다.", trigger: "blur" },
  ],
      ...
      try {
        ...
        if (response && response.success) {
          ElNotification.success({
            title: "성공",
            message: response.message || (isEditMode.value ? "객실 정보가 수정되었습니다." : "객실이 등록되었습니다."),
          });
