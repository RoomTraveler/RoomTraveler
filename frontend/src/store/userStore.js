...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    const _tokens = ref({ access_token: null, refreshToken: null });
    ...
    async function login(email, password) {
      ...
      try {
        ...
        _tokens.value = { access_token: null, refreshToken: null };
        user.value = null;
        sessionStorage.removeItem("user");
        return { success: false, error: error.value };
      } finally {
    ...
    const refresh = async () => {
      try {
        const refreshTokenValue = _tokens.value?.refreshToken;
        if (!refreshTokenValue) {
          console.error("[UserStore] No refresh token found in _tokens state. Logging out.");
          await logout();
          return { success: false, error: "No refresh token available" };
        }
        console.log("[UserStore] Attempting to refresh token with:", refreshTokenValue);
        ...
        // ##### 추가된 로그 시작 #####
        console.log("[UserStore] Raw response from /api/user/refresh:", JSON.parse(JSON.stringify(response)));
        // ##### 추가된 로그 끝 #####
        ...
        if (response.data && response.data.accessToken && response.data.refreshToken) {
          ...
          console.log("[UserStore] Tokens refreshed and updated in store:", JSON.parse(JSON.stringify(_tokens.value)));
          return { success: true };
        } else {
          console.error("[UserStore] Invalid response data from /api/user/refresh:", response.data);
          await logout();
          return { success: false, error: "Invalid token refresh response" };
        }
      } catch (refreshError) {
        console.error("[UserStore] Error during refresh API call or subsequent logout. RefreshError:", refreshError);
        if (!_isLoggingOut) {
          await logout();
        }
