        ...
        <div class="row" style="height: 70vh">
          <div class="col-md-4">
            ...
            <ul class="list-group mb-2" style="max-height: 500px; overflow-y: auto">
              <li v-for="spot in spots" :key="spot.no">
                <label>
      ...
      <div class="col-lg-4">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <button type="button" class="btn btn-success w-100" @click="savePlan">
              선택한 장소 저장
            </button>
          </div>
        </div>
      ...
      window.kakao.maps.load(() => {
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition((pos) => {
            ...
            map.value.setCenter(window.kakao.maps.LatLng(latitude, longitude))
          })
        }
      })
    }
...
const updateMap = (spots) => {
  try {
    ...
    if (!window.kakao || !window.kakao.maps || !map.value) {
    ...
    spots.forEach((info, i) => {
      ...
      const iwContent = `<div class="custom-infowindow">
          ...
          <button class="btn btn-sm btn-primary add-spot" data-id="${info.contentId}" 
            data-title="${info.title}" data-lat="${info.latitude}" data-lng="${info.longitude}">
            일정에 추가
          </button>
        </div>`

      const iwPosition = new window.kakao.maps.LatLng(info.latitude, info.longitude)
      ...
      window.kakao.maps.event.addListener(marker, 'click', () => {
        if (currentOpenIndex === i) {
          ...
          if (currentOpenIndex !== null) {
            infoWindows[currentOpenIndex].close()
          }
          infoWindows[i].open(map.value, marker)
          currentOpenIndex = i
        }
      })
